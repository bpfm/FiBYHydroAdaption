...
//static int LocalLenType[6]; - no longer need this, Alan
	...
	if(P[i].submark >= 0 && P[i].submark < HIGHBIT)
	  {
	    ...
	    len = subfind_unbind(ud, len, &lentype[0], &loop[0], &skipunbind[0]);
	    //      len = subfind_unbind(ud, len);
	    // It was necessary to modify the unbind variable to include lentype; see subfind_serial for the reasons why
...
void subfind_process_group_collectively(void)
{
  ...
  int lentype[6], totlen, totlentype;
...
#endif
		  for(i = 0; i < 6; i++)
		    candidates[k].lentype[i] = 0;	// Initiailise candidate.lentype

		  for(i = 0; i < LocalLen; i++)
		    {
		      Tail[ud[i].index] = nsubs;	/* we use this to flag the substructures */
		      candidates[k].lentype[P[ud[i].index].Type]++;	//lentype is created here, unsure about positioning - Alan
		    }
  ...
  for(i = 1, offset[0] = 0; i < NTask; i++)
    offset[i] = offset[i - 1] + countlist[i - 1];	//Offset is created here - i guess i'll have to also define offsettype here - Alan
  ...
  for(master = 0, subnr = 0; master < NTask; master++)
    {
      ...
      for(k = 0, totlen = 0, totlentype = 0; k < ncand; k++)
	{
	  if(ThisTask == master)
	    {
	      ...
	      for(j = 0; j < 6; j++)
		{
	      ...
	      totlen += len;
	      printf("check: totlen = %d while totlentype = %d \n", totlen, totlentype);
	  ...
	  MPI_Bcast(&len, sizeof(len), MPI_BYTE, master, MPI_COMM_WORLD);
	  // Attempting to modify the MPI call to return lentype as well - Alan
	  MPI_Bcast(lentype, sizeof(lentype), MPI_BYTE, master, MPI_COMM_WORLD);
	  ...
	  if(len > 0)
	    {
	      ...
	      LocalLen = 0;
	      //Added this loop to set LocalLenType to zero for this group - Alan
	      //              for(i = 0; i < 6; i++)
	      //        LocalLenType[i] = 0;
	      // Don't need this locallentype it seems
	      ...
	      else
		{
		  for(i = 0, p = candidates[k].head; i < candidates[k].len; i++)
		    {
		      subfind_distlinklist_add_bound_particles(p, nsubs);	//LocalLen is increased here so i've added LocalLenType - Alan
		      p = subfind_distlinklist_get_next(p);
		    }
	      ...
	      if(((GrNr - 1) % NTask) == ThisTask)
		{
		  ...
		  //How do i get LocalLenType to now get reduced into lentype like LocalLen is given to len ??
		  if(subnr == 0)
		  ...
		  if(subnr == 0)
		    for(j = 0; j < 6; j++)
		      {
			if(j == 0)
			  SubGroup[Nsubgroups].OffsetType[j] = Group[grindex].Offset;	// This means that one must read in the ParticleID list from the FOF group files
		  ...
		  if(SubGroup[Nsubgroups].Offset != SubGroup[Nsubgroups].OffsetType[0])
		    printf("Problem! SubGroup[%d].Offset = %d while (SubGroup[%d].OffsetType[0] = %d) \n",
			   Nsubgroups, SubGroup[Nsubgroups].Offset, (Nsubgroups),
			   SubGroup[Nsubgroups].OffsetType[0]);
  ...
  do
      ...
      if(phaseflag == 0)
	{
	  ...
	  for(i = 0, minindex = -1, minpot = 1.0e30; i < num; i++)
	    {
	      p = d[i].index;	//Changed this to 'p' rather than write d[i].index each time - Alan
      ...
      for(i = 0; i < num; i++)
	{
	  ...
	  if(P[part_index].Type == 0)
	    {
	      u =
		SphP[part_index].Entropy / GAMMA_MINUS1 * pow(SphP[part_index].d.Density * a3inv,
							      GAMMA_MINUS1);
	      P[part_index].v.DM_BindingEnergy += u;
	    }
...
{
  int i, j, p, part_index, *npart, numtot, mbid, nhalf, offset;
  ...
  for(i = 0, minindex = -1, minpot = 1.0e30; i < num; i++)
    {
      p = d[i].index;		// Added this line for simplicity- Alan
      if(P[p].Type > 0)		//Don't want gas particle being minimum potential point - Alan
	if(P[p].u.DM_Potential < minpot || minindex == -1)
	  {
	    minpot = P[p].u.DM_Potential;
	    minindex = p;
  ...
  *totmass = mass;
  ...
  for(i = 0, massloc = 0; i < num; i++)
      ...
      for(j = 0; j < 3; j++)
	{
...
#endif
	  dx[j] = All.Time * ddxx;	// dx is propto a^1 and h^0
	  dv[j] = vel_to_phys * (P[part_index].Vel[j] - v[j]);	// dv is propto a^1 and h^-1
	  dv[j] += H_of_a * dx[j];
      ...
      loc_rr_list[i].r = sqrt(loc_rr_list[i].r);	//loc_rr_list[i].r is propto scale factor, so is proper distance
    }
  ...
  *veldisp = sqrt(disp / (3 * mass));	/* convert to 1d velocity dispersion */
  ...
  spin[0] = lx / mass;		// spin is propto a^2 and h^-1
  spin[1] = ly / mass;
  spin[2] = lz / mass;
  ...
  if(ThisTask == mincpu)
    *halfmassrad = loc_rr_list[nhalf].r;	//halfmassrad is proper distance as loc_rr_list[i].r is proper
...
void subfind_distlinklist_add_bound_particles(long long index, int nsub)
{
  ...
  task = (index >> 32);		// Pick out the processor number on which this particle 'index' resides
  i = (index & MASK);

  if(ThisTask == task)		// Only if the particle is local to the processor
    {
      if(Tail[i] == nsub)	/* consider only particles not already in substructures */
	{
	  ud[LocalLen].index = i;	//Creating a local ID list -> P ID list
	  LocalLen++;
	  //      LocalLenType[P[i].Type]++; //Is this ud[LocalLen].index / i actually the correct one to use here? - don't need it
	}
    }
  else
    {
...
{
  if(((sort_r2list *) a)->xproj < ((sort_r2list *) b)->xproj)
    return -1;
...
{
  if(((sort_r2list *) a)->yproj < ((sort_r2list *) b)->yproj)
    return -1;
...
{
  if(((sort_r2list *) a)->zproj < ((sort_r2list *) b)->zproj)
    return -1;
