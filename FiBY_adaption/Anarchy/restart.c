      ...
      if(ThisTask == (masterTask + groupTask))	/* ok, it's this processor's turn */
	{
	  ...
	  if(modus)		/* read */
	      ...
	      if(All.PartAllocFactor != save_PartAllocFactor)
		{
		  ...
		  if(All.TotN_star == 0)
		    All.MaxPartStar =
		      (int) (All.PartAllocFactor * (All.TotN_gas / NTask) * All.Generations *
			     All.StarAllocFactor);
		  else
		    All.MaxPartStar = (int) (All.PartAllocFactor * (All.TotN_star / NTask +
								    (All.TotN_gas / NTask) * All.Generations *
								    All.StarAllocFactor));
#endif
	  ...
	  if(N_star > 0)
		...
		{
		  printf
		    ("STAR: it seems you have reduced(!) 'PartAllocFactor * StarAllocFactor' below the value of %g needed to load the restart file.\n",
