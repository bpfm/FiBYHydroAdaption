...
int bg_bh_thermal_feedback_evaluate(int target, int mode, int *nexport, int *nsend_local)
{
  ...
  critical_energy = All.BlackHoleMinHeatTemp * (ngb_mass/(float)num_ngb) * All.BlackHoleNumberOfNeighboursToHeat / u_to_temp_fac;

  if (bh_energy > critical_energy) {
    nheat = bh_energy/critical_energy;
    p = nheat/(float)num_ngb;
  } else {
    return 0;
  }
  ...
  while(startnode >= 0)
      ...
      while(startnode >= 0)
	{
	  ...
	  numngb_inbox = bg_bh_thermal_feedback_ngb_treefind(&pos[0], h, target, &startnode, mode, nexport, nsend_local);
