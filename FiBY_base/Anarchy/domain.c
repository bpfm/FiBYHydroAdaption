...
/* #define MIN_PARTS_PER_SECOND 10000 */
/* #define MIN_NUM_FORCE_UPDATE   100 */
/* #endif */
...
#ifdef BG_SFR
static int *list_N_star, *list_loadstar, *local_toGoStar, *toGoStar;
#endif
...
static int first_domaindecomp_flag = 1;
...
#ifdef BG_SFR
static int *domainCountStar;	/*!< a table that gives the total number of star particles held by each processor */
#endif
...
/* #ifdef FORCE_DOMAIN_DECOMPOSITION */
/*   if(PartsPerSecond < MIN_PARTS_PER_SECOND && GlobNumForceUpdate > MIN_NUM_FORCE_UPDATE) */
/*     { */
/*       All.NumForcesSinceLastDomainDecomp = (long long) (1 + All.TotNumPart * All.TreeDomainUpdateFrequency); */

/*       if(ThisTask == 0) */
/* 	{ */
/* 	  printf("Forced domain decomposition: part/sec=%f, Nf=%d\n", PartsPerSecond, GlobNumForceUpdate); */
/* 	  fflush(stdout); */
/* 	} */
/*     } */
/* #endif */
  ...
  if(All.NumForcesSinceLastDomainDecomp >= All.TotNumPart * All.TreeDomainUpdateFrequency
      ...
      do
	{
	  ...
	  all_bytes += bytes;
#ifdef BG_SFR
	  toGoStar = (int *) mymalloc(bytes = (sizeof(int) * NTask * NTask));
	  all_bytes += bytes;
#endif
	  ...
	  all_bytes += bytes;
#ifdef BG_SFR
	  local_toGoStar = (int *) mymalloc(bytes = (sizeof(int) * NTask));
	  all_bytes += bytes;
#endif
	  ...
	  all_bytes += bytes;
#ifdef BG_SFR
	  list_N_star = (int *) mymalloc(bytes = (sizeof(int) * NTask));
	  all_bytes += bytes;
#endif
	  ...
	  all_bytes += bytes;
#ifdef BG_SFR
	  list_loadstar = (int *) mymalloc(bytes = (sizeof(int) * NTask));
	  all_bytes += bytes;
#endif
	  ...
	  all_bytes += bytes;
#ifdef BG_SFR
	  domainCountStar = (int *) mymalloc_msg(bytes = (MaxTopNodes * sizeof(int)), "domainCountStar");
	  all_bytes += bytes;
#endif
...
#ifdef BG_SFR
	  maxLoadstar = (int) (All.MaxPartStar * REDUC_FAC);
#endif
...
int domain_decompose(void)
{
  ...
  do
    {
#if defined(PEDANTIC_MEMORY_CEILING) || defined(DOMAIN_MEMORY_CEILING)
...
#ifdef BG_SFR
      exchange_limit += (NumPart + 2) * sizeof(struct star_particle_data);
#endif
#endif
...
int domain_check_memory_bound(void)
{
  ...
  for(ta = 0; ta < NTask; ta++)
	...
	max_sphload = sphload;
#ifdef BG_SFR
      if(starload > max_starload)
	max_starload = starload;
#endif
    }
...
#ifdef BG_SFR
  if(max_starload > maxLoadstar)
    {
      if(ThisTask == 0)
	{
	  printf("desired starallocfactor=%g  (Stars) (limit=%d, needed=%d)\n",
		 (max_starload * All.StarAllocFactor) / maxLoadstar, maxLoadstar, max_starload);
	}
	  ...
	  if(count_recv_star[target] > 0)
	    {
	      ...
	      MPI_Irecv(Key + offset_recv_star[target] + NumPart + count_get_sph - N_star,
			count_recv_star[target] * sizeof(peanokey), MPI_BYTE, target, TAG_KEY_STAR,
	  ...
	  if(count_star[target] > 0)
	    {
	      ...
	      MPI_Isend(keyBuf + offset_star[target], count_star[target] * sizeof(peanokey),
			MPI_BYTE, target, TAG_KEY_STAR, MPI_COMM_WORLD, &requests[n_requests++]);
	  ...
	  if(count_star[target] > 0 || count_recv_star[target] > 0)
	    {
	      MPI_Sendrecv(partBuf + offset_star[target], count_star[target] * sizeof(struct particle_data),
			   ...
			   P + offset_recv_star[target] + NumPart + count_get_sph - N_star,
			   count_recv_star[target] * sizeof(struct particle_data), MPI_BYTE, target,
			   TAG_PDATA_STAR, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	      MPI_Sendrecv(starBuf + offset_star[target] - offset_star[0],
			   count_star[target] * sizeof(struct star_particle_data), MPI_BYTE, target,
			   TAG_STARDATA, StarP + offset_recv_star[target],
			   count_recv_star[target] * sizeof(struct star_particle_data), MPI_BYTE, target,
			   TAG_STARDATA, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

	      MPI_Sendrecv(keyBuf + offset_star[target], count_star[target] * sizeof(peanokey),
			   MPI_BYTE, target, TAG_KEY_STAR,
			   Key + offset_recv_star[target] + NumPart + count_get_sph - N_star,
			   count_recv_star[target] * sizeof(peanokey),
...
#ifdef BG_SFR
  package += sizeof(struct star_particle_data);
#endif
  if(package >= nlimit)
    endrun(212);
...
#ifdef BG_SFR
		  if((ntoomany = list_N_star[ta] + count_toget_star - count_togo_star - All.MaxPartStar) > 0)
		    {
		      if(ThisTask == 0)
			{
			  printf
			    ("exchange needs to be modified because I can't receive %d Star-particles on task=%d\n",
			     ntoomany, ta);
			  if(flagsum > 25)
			    printf("list_N_star[ta=%d]=%d  count_toget_star=%d count_togo_star=%d\n",
...
{
  int i, j, sub, ngrp, imax1, imax2, ntopnodes_before;
  ...
  do
	...
	return errsum;
      ...
      if(first_domaindecomp_flag)
	domain_update_topNodesCost();
      else
	break;
    }
  while(NTopnodes > ntopnodes_before);

  first_domaindecomp_flag = 0;
...
void domain_update_topNodesCost(void)
{
  ...
  double *loc_topNodesCost, *glob_topNodesCost;

  int *loc_topNodesCount, *glob_topNodesCount;

  loc_topNodesCost = (double *) mymalloc(NTopnodes * sizeof(double));
  glob_topNodesCost = (double *) mymalloc(NTopnodes * sizeof(double));
  loc_topNodesCount = (int *) mymalloc(NTopnodes * sizeof(int));
  glob_topNodesCount = (int *) mymalloc(NTopnodes * sizeof(int));

  for(i = 0; i < NTopnodes; i++)
    {
      loc_topNodesCost[i] = 0;
      loc_topNodesCount[i] = 0;
    }
  ...
  for(n = 0; n < NumPart; n++)
	...
	no = topNodes[no].Daughter + (Key[n] - topNodes[no].StartKey) / (topNodes[no].Size / 8);
      ...
      loc_topNodesCost[no] += domain_particle_costfactor(n);
      loc_topNodesCount[no]++;
    }

  MPI_Allreduce(loc_topNodesCost, glob_topNodesCost, NTopnodes, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  MPI_Allreduce(loc_topNodesCount, glob_topNodesCount, NTopnodes, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

  for(i = 0; i < NTopnodes; i++)
    if(topNodes[i].Daughter < 0)
      {
	topNodes[i].Cost = glob_topNodesCost[i];
	topNodes[i].Count = glob_topNodesCount[i];
      }

  myfree(glob_topNodesCount);
  myfree(loc_topNodesCount);
  myfree(glob_topNodesCost);
  myfree(loc_topNodesCost);
}
...
#ifdef BG_SFR
  local_DomainCountStar = (int *) mymalloc_msg(NTopnodes * sizeof(int), "local_DomainCountStar");
#endif
