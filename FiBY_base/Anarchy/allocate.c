...
void allocate_memory(void)
{
  ...
  if(All.MaxPartSph > 0)
	...
	printf("Allocated %g MByte for storage of SPH data.\n\n", bytes_tot / (1024.0 * 1024.0));
      ...
      if(!
	 (StarP =
	  (struct star_particle_data *) mymalloc(bytes =
						 All.MaxPartStar * sizeof(struct star_particle_data))))
	{
	  printf("failed to allocate memory for `StarP' (%g MB).\n", bytes / (1024.0 * 1024.0));
	  endrun(1);
	}
      bytes_tot += bytes;

      if(ThisTask == 0)
	printf("Allocated %g MByte for storage of star data.\n\n", bytes_tot / (1024.0 * 1024.0));
#endif
    }
}
